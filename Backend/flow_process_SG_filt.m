function [dts, pkpk] = flow_process_SG_filt(upvolts, downvolts, time, N_interp, starts, stops)

%     [dts, corr_x, corr] = flow_process_SG_filt(upvolts, downvolts, time, N_interp, starts, stops)
...
%     differences due to the flow.
    ...
    dts = zeros(1,length(starts));
    pkpk = nan(size(starts));
    %start looping through arrivals
    for arr_i = 1:length(starts)       
        ...
        pkpk(arr_i) = max(arr1_int) - min(arr1_int);
